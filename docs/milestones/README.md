...
|---|---|---|---|---|
| M0 | [`M0_IMPLEMENTATION_PLAN.md`](./M0_IMPLEMENTATION_PLAN.md) | Week 1–2 | Skeleton, `services/api` + `apps/web` scaffolded, Ruleset, `docker compose up` on 2 OSes | **Phase 2 complete** — `M0-1..M0-13` repo foundation (`v0.0.1`), `M0-14..M0-20` backend scaffolding (`v0.0.2`), `M0-21..M0-34` remaining in Backlog |
...
- Tagging: all tags use `v` prefix — `v0.0.x` for internal checkpoints, `v0.1.0` (M0), `v0.2.0` (M1) … `v1.0.0` (M6 defense) for milestone releases. `Admin`-only via **Tag protection** `Settings → Tags → New tag protection rule` pattern `v*` (see `docs/PROJECT_CONTEXT.md:300`). Current: `v0.0.0` skeleton, `v0.0.1` Phase 1, `v0.0.2` Phase 2.

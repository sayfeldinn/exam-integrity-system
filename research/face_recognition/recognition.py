...
CONSECUTIVE_MISMATCH_REQUIRED = 2
...
class RecognitionSession:
    ...
    def check_frame(self, frame) -> dict:
        ...
        if stored is None:
            raise RuntimeError(
                f"Student {self.student_id} is not registered in the database."
            )
        ...
        score = cosine_similarity(stored_embedding, current_embedding)
    ...
    def _register_violation(self, score: float):
        # Placeholder: will link to the real violations table
        print(
            f"[VIOLATION] Student {self.student_id}: "
            f"Face mismatch detected (score={score:.2f})"
        )


def run_periodic_check(student_id: str, interval_sec: int = 45, iterations: int = 5):
    """Manual test: periodic check every `interval_sec` seconds."""
